...
- **Local `.env.local` has no `SUPABASE_SERVICE_ROLE_KEY`.** Every admin route (`getSupabaseAdmin()`) throws locally as a result - not new, affects every `/api/ops/*` route, not just Phase 3's. `npm run build` / `tsc --noEmit` still fully verify locally; anything that actually touches the service-role client needs the deployed dev Render service instead.

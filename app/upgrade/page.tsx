    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--txt)', fontFamily: 'inherit' }}>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '56px 24px 80px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 48 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 11, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--accent)', background: 'rgba(139,92,246,.1)', border: '0.5px solid rgba(139,92,246,.3)', borderRadius: 20, padding: '4px 14px', marginBottom: 20 }}>
            Pro Plan
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16, marginBottom: 36 }}>
          ...
          {/* Pro card */}
          <div style={{ borderRadius: 16, padding: '24px 28px', border: '0.5px solid rgba(139,92,246,.5)', background: 'linear-gradient(160deg, rgba(139,92,246,.1) 0%, var(--bg1) 60%)', position: 'relative' }}>
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 9 }}>
              {PRO_FEATURES.map(f => (
                <li key={f} style={{ fontSize: 13, color: 'var(--txt2)', display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                  <span style={{ color: '#a78bfa', fontWeight: 700, flexShrink: 0, marginTop: 1 }}>✓</span> {f}
                </li>
              ))}
            </ul>
          </div>

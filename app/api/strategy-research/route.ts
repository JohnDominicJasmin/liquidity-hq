...
export async function POST(req: NextRequest) {
  ...
  const { used, limit } = await checkToolUsage(token, authData.user.id, 'strategyResearch');
  if (used >= limit) {
    return NextResponse.json(
      { error: `Daily limit of ${limit} strategy research runs reached.`, code: 'RATE_LIMIT' },
      { status: 429 },
    );
  }
  ...
  await incrementToolUsage(token, authData.user.id, 'strategyResearch', used);

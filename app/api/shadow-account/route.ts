...
export async function POST(req: NextRequest) {
  ...
  if (!(await hasProFeatures(token, authData.user.id))) {
    return NextResponse.json({ error: 'PRO_REQUIRED', message: 'Shadow Account is a Pro feature.' }, { status: 403 });
  }
  ...
  await recordAiCall({ userId: authData.user.id, callType: 'shadow_account', model: data.model ?? 'grok-4.3', usage: data.usage });

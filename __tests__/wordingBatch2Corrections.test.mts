 ...
 * 36, 37, 38, 39, 40, 41, 42, 43, 44, 54, 57). Two contracts, independent of
 ...
 *  (b) none of the corrected keys' EN values, and none of the raw Telegram
 *      message strings in app/api/telegram/alert/route.ts (items 42/57 are
 *      partly CODE, not labels, so no migration touches these - per
 *      PM/DevOps: "Telegram strings ... are code, not labels, so no
 ...
 * red: on PR A's own branch, once the migration exists, ALL FOUR subtests
 * below must actually RUN and pass - a skip there (e.g. from a filename
 * that doesn't match either pattern) counts as a fail at review, not a
 * pass. Match patterns, don't guess a single exact filename.
 *
 ...
 *
 * Part (b) also covers app/api/telegram/alert/route.ts, which doesn't
 * depend on the migration at all - but it's gated behind the SAME skip as
 * the other three, not run standalone, for the same pre-push reason above
 * (a hard-red assertion here blocks `git push` just as much as one in part
 * (a) would). Verified by running it manually, outside the pushed suite:
 * it already fails for real, on dev, right now - "institutional
 * accumulation" / "institutional distribution" (whale alert strings, ~line
 * 831-869) and one all-caps "Do NOT" (~line 1338, the funding-signal
 * short-squeeze action text). PM/DevOps confirmed items 42/57 cover these
 * and will make sure Dev's PR A fixes them - this subtest is what turns
 * that into an enforced check once PR A lands.
 */
...
test('wording batch 2 (#1309 PR A, items 36-44/54/57): migration matches labelDefaults, banned phrases gone',
  { skip: SKIP }, async (t) => {
    ...
    await t.test("none of the migration's corrected keys still contain a banned phrase", () => {
      const rows = loadMigrationRows(migrationFiles);
      for (const [key, value] of rows) {
        for (const { name, re } of BANNED) {
          assert.ok(!re.test(value), `labels migration key "${key}" still contains ${name}: "${value}"`);
        }
      }
    });

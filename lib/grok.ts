...
export function buildPrompt(ctx: GrokContext): string {
  return [
    ...
    '  • CVD bullish divergence in a selloff = absorption trap. Buyers absorbing into falling price often precedes the next leg down.',
    ...
    '  • DEFAULT to FLAT (wait) when momentum is bearish but no strong short catalyst either — do NOT force a SHORT just because it is falling.',
    '  • Call SHORT only when: momentum is bearish AND you find a specific negative catalyst AND OI trend confirms new shorts entering.',
    '  • Call LONG only when: you find a specific hard reversal catalyst (Trump tweet, Fed pivot, major news reversal) that overrides the selloff.',
    ...
    '',
    '=== FLAT = "WAIT FOR A BETTER SETUP" — THIS IS A HIGH-QUALITY SIGNAL ===',
    'FLAT does NOT mean you have no opinion. FLAT means the current conditions do not justify a trade yet.',
    'Use FLAT (with 65–85% confidence) when:',
    '  • Bearish momentum regime confirmed but no strong directional catalyst in either direction',
    '  • CVD bullish divergence present during a sustained selloff — absorpttion visible but selling not stopped',
    '  • RSI oversold AND price still making lower lows — oversold can get more oversold',
    '  • Conflicting signals: bearish structure + bullish divergence + no macro catalyst = no edge',
    '  • Volume is drying up mid-selloff — could break either way, no conviction trade available',
    '  • You would need to chase entry or risk too much to define a clean stop — wait for structure',
    'FLAT reasoning must explain WHAT to watch for before taking a trade (e.g., "Wait for price to reclaim $X with volume, or for selling to exhaust with a clear reversal candle at support").',
    'NEVER output FLAT at low confidence (< 50%) — if you say FLAT, be confident that waiting IS the right trade.',
    '',

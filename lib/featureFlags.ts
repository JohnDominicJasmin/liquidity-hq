...
export type FeatureFlag = 'grok' | 'telegram' | 'signups';
...
const DEFAULTS: Record<FeatureFlag, boolean> = { grok: true, telegram: true, signups: true };
...
export async function getFeatureFlags(): Promise<Record<FeatureFlag, boolean>> {
  ...
  try {
    ...
    data = { grok: v?.grok ?? true, telegram: v?.telegram ?? true, signups: v?.signups ?? true };

...
export default function WatchlistFeed() {
  ...
  if (watchlist.length === 0) {
    return (
      <div style={{ padding: '12px 0', fontSize: 'var(--fs-caption)', color: 'var(--txt3)' }}>
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4, gap: 6 }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 }}>
                ...
                <span style={{ fontSize: 'var(--fs-caption)', fontWeight: 700, color: 'var(--txt)', letterSpacing: '.03em' }}>
                  {id.toUpperCase()}
                </span>
              </span>
              {d?.price && (
            ...
            {/* Price */}
            <div style={{ fontSize: 'var(--fs-data)', fontWeight: 700, color: 'var(--txt)', marginBottom: 2 }}>
              {d?.price ? '$' + fmtPrice(d.price, dec) : '-'}
            </div>

            {/* Change + sparkline */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
              <span style={{ fontSize: 'var(--fs-caption)', fontWeight: 600, color: up ? '#34d399' : '#f87171' }}>
            ...
            {/* Signal */}
            {sig ? (
              <div style={{ fontSize: 'var(--fs-caption)', color: sig.col, lineHeight: 1.3 }}>{sig.text}</div>
            ) : (
              <div style={{ fontSize: 'var(--fs-caption)', color: 'var(--txt3)' }}>No signal</div>
            )}
          </div>
        );
      })}

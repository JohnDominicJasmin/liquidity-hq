...
for (const locale of LOCALES) {
  test.describe(`locale=${locale}`, () => {
    test('the About page Contact card shows the served wording and names the address', async ({ browser, request }) => {
      const expected = (await servedLabels(request, locale)).ABOUT_CONTACT_BODY;
      expect(expected, `ABOUT_CONTACT_BODY is not served for locale=${locale}`).toContain(SUPPORT_EMAIL);
    ...
    test('a banned, signed-out visitor sees the served suspension wording naming the address', async ({ browser, request }) => {
      const expected = (await servedLabels(request, locale)).AUTH_GATE_BANNED_DESC;
      expect(expected, `AUTH_GATE_BANNED_DESC is not served for locale=${locale}`).toContain(SUPPORT_EMAIL);

...
export const PROD_READONLY_SPECS: ReadonlyArray<{ spec: string; needs: Needs; note?: string }> = [
  ...
  { spec: 'batch3-round1-label-rows', needs: 'none', note: 'GETs /api/labels only; red on prod until the #1386 label rows are applied there' },
  { spec: 'briefing-funding-navigation', needs: 'none', note: 'anonymous GETs; live data, so some tests skip when there is nothing to act on' },
...
export const PROD_EXCLUDED: Readonly<Record<string, string>> = {
  ...
  'onboarding-wizard-focus-and-save': 'shows the page an INCOMPLETE onboarding row and answers every write by route stub; safe only while a stub matches, and a missed match would rewrite a real account\'s onboarding state',
  'settings-honest-controls': 'answers its /api/settings, push-subscribe and Telegram writes by route stub; safe only while a stub matches, and a missed match would save to a real account',

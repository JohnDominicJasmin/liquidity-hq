...
> two screens, not two drafts.
>
> This file was committed at `design-handoff-dir/design_handoff_arena/README.md`
> and **deleted on 2026-09-01 by `ed219ccd`**, which replaced the whole directory
> with the designer's current bundle. The bundle had no Arena README, so the path
> went away with the folder. That commit dissolved `design_handoff_arena/` and
> moved `specs/arena.md` across deliberately — the README was collateral, not a
> decision. Restored verbatim from `3e604cb6` on 2026-09-06 under #889, with the
> one paragraph that asserted external state corrected in place and marked.
>
> **#853 — rebuild `ArenaTerminal.tsx` — is parked, and this is its handoff.**
> It spent five days absent from the repository. Read `specs/arena.md` with it;
> the spec adjudicates.
...
> When this paragraph was written it still held the originals — which is exactly
> how this drift stayed invisible, since nothing imported it. See #518.
>
> **Corrected on restore, 2026-09-06 (#889):** #518 is closed and
> `lib/terminalTokens.ts` now carries all three amended values. The sentence
> above is kept in the past tense rather than deleted, because the fact that a
> handoff and its token file disagreed for a week is the reason this block
> exists. **The token file has also grown past the 15 named here** — `--amber`
> (#542), `--fr-slight-long` (#559/#561), `--txt-dash` (#559), plus a light
> palette. Read the count from the file, never from this README.

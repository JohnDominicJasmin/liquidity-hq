...
**Current total: 1836 label rows**, identical in both `lhq_labels` (prod) and `lhq_dev_labels` (dev). 75 files fully migrated: 27 pages (about, alerts, arena, backtest, briefing, calc, correlation, dashboard, disclaimer, econ-calendar, funding, hours, journal, liq, live-tracking, login, markets, news, not-found, playbook, prices, privacy, research, scanner, settings, terms, upgrade) + 48 components (AbsorptionDetector, AccumulationTracker, AlertOutcomes, AuthGate, BtcRiskLevel, CoinHeatmap, CoinMultiSelect, ConfluenceScore, CycleChart, CycleDayCounter, DcaCalc, DistributionTracker, DrawdownChart, DryPowder, EconCalendarWidget, FundingCostCalc, GexTable, GlobalMacroContext, GrokSignalChart, HigherTfMoveBadge, HypothesisTracker, LabelsProvider, LanguageSelect, LanguageSync, LiqHeatmap, LiquidationCalc, MarketConditionsWidget, MarketRead, MarketStructure, MultiTFAlignment, MultiTFSqueezeView, NavDrawer, OnChainScore, PageHint, PnLCalc, PositionSizer, RiskRewardCalc, SessionCountdown, SettingsModal, SetupScanner, SignalAccuracy, StopLossZone, ThemeChips, UpgradeGateModal, UsageMeter, UsageRings, VolatilityRegime, WhaleTradesFeed). Sparkline/Sparkline24h intentionally skipped — pure SVG renderers, zero hardcoded text.
...
Fifteen i18n-breaks-logic bugs caught and fixed so far, across two recurring classes plus a third variant of the first — always worth explicitly warning delegated agents about all of them (the wave-5+ agent prompts now include this warning verbatim, keep doing that):

...
| `LiquidityHQ` | `https://liquidity-hq.onrender.com/api/telegram/alert` | every 5 min (`*/5 * * * *`, tz Asia/Manila) | **Active**, succeeding | none (no custom headers sent) | The actual Telegram/Web Push alert scanner — RSI, EMA crosses, whale trades, OI spikes, news, fear/greed. See `ARCHITECTURE.md` §7. |
...
- **7 of the 11 `DASHBOARD_SECTIONS` toggles in Settings are inert** (`accumulation`, `distribution`, `session`, `catalysts`, `gex`, `macro`, `commandments` — `raid_meter` was fixed 2026-07-19). Toggling them in Settings does nothing; `/dashboard` only actually gates 4 of the 11. See the `project_lhq_tier2_backlog` memory for detail. Deferred by the user, not yet scoped for a fix.

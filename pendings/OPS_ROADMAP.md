...
- **Two separate Supabase projects, one per tier:** `liquidity-hq-prod` (ref `qdpwhnvmhqgzijuwopso`) = **prod only**; `liquidity-hq-dev` (ref `wdtjhrilakoitfcezxpx`) = **local dev AND the deployed `liquidity-hq-dev`** Render service (same project, both dev contexts - note the Supabase project and the Render service share this exact name, they are not the same system). CORRECTED 2026-07-20 - earlier docs in this file said the prod project covered local dev too; that was wrong and caused a real debugging detour (local `.env.local` pointed at prod, which only has `lhq_dev_admin_users`/`admin_audit_log`/`app_config` - none of the app's other `lhq_dev_*` tables exist there). `.env.local` now correctly points at the dev project. A "table not found in schema cache" locally is usually this exact mistake, not a cache lag. (See `docs/INFRASTRUCTURE.md` §4 for the current, corrected version.)
...
- **Local admin routes now work.** `.env.local` has `SUPABASE_SERVICE_ROLE_KEY` filled in (`liquidity-hq-dev` project's key) as of 2026-07-20 - every `/api/ops/*` route and `/ops/config` itself are fully testable on `localhost:3000`, no dev-service hours needed. Get a fresh key from `liquidity-hq-dev` -> Settings -> API -> Legacy API keys -> `service_role`, never `liquidity-hq-prod`'s.

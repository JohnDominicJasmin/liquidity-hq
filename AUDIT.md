...
### SYS-7 — All font sizes fixed `px`, none `rem` `[Medium, a11y]` — 🔵 deferred, not priority
Zero `rem` in `globals.css`; inline styles px too. Text ignores browser font-size preference / resists zoom-reflow. See §7. Scope: ~580 CSS + ~678 inline declarations across the whole codebase - real fix but same size/risk as a full design-token migration, not a quick win. Explicitly deprioritized by product owner (2026-07-17) - app is functional today, revisit as its own dedicated pass later.

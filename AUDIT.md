...
| **Medium** | ⚠️ P2-3 (`/learn` hierarchy) — **mostly a mis-measurement, see correction**; only body 13→14px bumped, ✅ P2-5 (news-dismiss tap target) — **NOT A BUG, mis-measured** (button is 34×28px, passes AA), ✅ P2-6 (mobile dashboard horizontal overflow) — **FIXED**, root cause was the off-canvas nav-drawer inflating `scrollWidth`, one visual-verification caveat noted |
...
### P2-6 — Mobile dashboard scrolls horizontally `[Medium]` — ✅ FIXED 2026-07-19, one verification caveat below
...
**One caveat, disclosed rather than hidden:** while investigating, the preview tab's `document.hidden` flipped to `true` mid-session (lost visibility/focus, likely from switching to the other browser tool) — Chromium throttles CSS-transition rendering for hidden documents, so a `getComputedStyle().transform` read on the nav-menu appeared stuck mid-transition when manually toggling the drawer open via its class. Ruled out a real regression through elimination before accepting the tooling explanation: forced the `open` class via raw DOM (bypassing React) with the same stuck result, confirmed the exact matching CSS rule (`.nav-drawer.open .nav-menu{transform:translateX(0)}`) is present, correctly specific, unlayered, and un-overridden in the served stylesheet, and confirmed zero inline style conflicts — nothing in the `overflow-x:hidden` change has any CSS mechanism to affect a *different* element's transform rule. Screenshot verification of the open/close *animation* itself could not be completed this session due to the tooling issue - flagging for a follow-up visual pass, not claiming it as done.

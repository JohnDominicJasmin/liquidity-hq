...
| **Medium** | ⚠️ P2-3 (`/learn` hierarchy) — **mostly a mis-measurement, see correction**; only body 13→14px bumped, ✅ P2-5 (news-dismiss tap target) — **NOT A BUG, mis-measured** (button is 34×28px, passes AA), 🔎 P2-6 (mobile dashboard horizontal overflow) — **culprit traced to the nav-drawer, still open (not a cheap fix)** |
...
### P2-6 — Mobile dashboard scrolls horizontally `[Medium]` — 🔎 culprit traced, STILL OPEN (not a cheap/safe fix)
Re-traced 2026-07-19. The marquee is **not** the cause — `.ticker-content` sits inside `.ticker-track{overflow:hidden}` *and* `.news-ticker{position:fixed;overflow:hidden}`, doubly clipped. The rightmost off-viewport content is the **nav-drawer / `.nav-menu`** (the hamburger menu, `position:absolute`, right edge measured ~843px) plus `.nav-search-bar` — i.e. the mobile nav drawer is laid out off-canvas but its extent still contributes to document scroll width (`html`/`body` both `overflow-x: visible`). **Why it's not fixed here:** (a) the pane's fractional-width rendering makes the exact overflow figure unreliable, so I can't cleanly confirm the fix reduces it to 0; (b) the real fix is in the `NavDrawer` component's closed-state positioning (clip/translate the drawer, or set `overflow-x:clip` on a wrapper) which needs the drawer's open/close mechanism understood and verified at a stable mobile width — not a one-line CSS nudge. Kept open for a focused pass. Earlier guess (marquee) explicitly ruled out.

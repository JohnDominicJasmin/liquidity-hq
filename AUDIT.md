...
| **Medium** | ⚠️ P2-3 (`/learn` hierarchy) — **mostly a mis-measurement, see correction**; only body 13→14px bumped, ✅ P2-5 (news-dismiss tap target) — **NOT A BUG, mis-measured** (button is 34×28px, passes AA), ✅ P2-6 (mobile dashboard horizontal overflow) — **FIXED + fully verified** (root cause: off-canvas nav-drawer inflating `scrollWidth`; drawer open/close confirmed working on a real phone) |

> **Re-measurement correction (2026-07-19).** The mobile/tablet numbers in this Pass were captured while both browser tools were degraded (claude-in-chrome resize dead; the preview pane wedged on screenshots and reported a *fluctuating fractional width* — e.g. innerWidth 492 while clientWidth was 390). On re-measuring the Medium items against a now-cooperative pane, **two were flaky-pane artifacts, not real defects**: P2-5 (the "12×12px" was the inner SVG, not the 34×28px button) and most of P2-3 (h2 is an 11px accent eyebrow, not 16px — the hierarchy is actually well-differentiated). Treat the exact pixel counts in the High findings (P2-1 contrast, P2-2 layout) as trustworthy — those were re-verified independently — but sanity-check any Medium/Low pixel measurement before acting on it.
| **Low** | ✅ P2-4 (10px sub-legible text) — **FIXED**, `--fs-xs` token 10→11px, ✅ P2-7 (RaidMeter orphan cell) — **FIXED**, 5th tile spans full width |
...
### P2-4 — Sub-11px text still present `[Low, a11y]` — ✅ FIXED + verified 2026-07-19
Traced to `--fs-xs: 0.625rem` (10px) — a token from the older 8-tier size scale (Pass 1 §7's addendum #24), still driving 6 real text call sites ([globals.css](app/globals.css): `.obw-seg-label`, `.obw-eyebrow`, `.obw-skip`, `.obw-footer-disc`, `.obw-footer-link` in the onboarding wizard) plus one duplicate hardcoded literal (`.ticker-see-all-label`). **Fix:** bumped `--fs-xs` to `0.6875rem` (11px, matching `--fs-micro`/`--fs-sm`, the app's own stated floor) and switched `.ticker-see-all-label` off its hardcoded literal onto the token. Verified live: `getComputedStyle(document.documentElement).getPropertyValue('--fs-xs')` returns `.6875rem` post-fix.
...
### P2-6 — Mobile dashboard scrolls horizontally `[Medium]` — ✅ FIXED + FULLY VERIFIED 2026-07-19 (incl. real device)
**Root cause, confirmed via `getBoundingClientRect`, not guessed:** the marquee is **not** the cause — `.ticker-content` sits inside `.ticker-track{overflow:hidden}` *and* `.news-ticker{position:fixed;overflow:hidden}`, doubly clipped (earlier guess explicitly ruled out). The actual source is `.nav-menu` (the off-canvas drawer content, global via `AppShell` → `NavDrawer` on every page): when closed it sits at `transform: translateX(100%)`. Measured directly: its transformed right edge landed **351px past the true viewport edge** — exactly matching its own width and the measured document overflow. `position:fixed` + `transform` descendants are a known browser edge case: giving the fixed `.nav-drawer` ancestor `overflow:hidden` **did not** stop this (verified — overflow was unchanged after that alone, kept anyway as harmless hygiene).

**Fix applied ([globals.css](app/globals.css)):** `overflow-x: hidden` on `html` — the standard, low-risk fix for this exact bug class (off-canvas drawer inflating `scrollWidth` via transform). This app has no legitimate horizontal-scroll surface, so blocking it at the root is safe.

**Verified, non-paint-dependent:** `window.scrollTo(200,0)` / `documentElement.scrollLeft` / `body.scrollLeft` all confirmed **unable to move the page horizontally** post-fix (were previously free to scroll ~93px on mobile) — this is the actual user-facing bug, fixed and directly confirmed. Zero console errors.

**Drawer open/close animation — confirmed on real device 2026-07-19.** While investigating, the preview tab's `document.hidden` flipped to `true` mid-session (lost visibility/focus, likely from switching to the other browser tool) — Chromium throttles CSS-transition rendering for hidden documents, so a `getComputedStyle().transform` read on the nav-menu appeared stuck mid-transition when manually toggling the drawer open via its class. Two automated retries (a fresh preview-pane server restart, and claude-in-chrome on a second pass) both hit the same visibility/no-op-resize limitation and couldn't render true mobile width to re-check visually. Ruled out a real regression through elimination before it mattered: forced the `open` class via raw DOM (bypassing React) with the same stuck read, confirmed the exact matching CSS rule (`.nav-drawer.open .nav-menu{transform:translateX(0)}`) is present, correctly specific, unlayered, and un-overridden in the served stylesheet, confirmed zero inline style conflicts — `overflow-x:hidden` has no CSS mechanism to affect a *different* element's transform rule. User confirmed live on prod, real phone: drawer opens and closes normally.

### P2-7 — RaidMeter 5-factor grid leaves an orphan cell `[Low]` — ✅ FIXED + verified 2026-07-19
RaidMeter has **5** factor tiles (Session, Day, Fear & Greed, Funding, Order Wall) laid out in a **2-column** grid at mobile/tablet → was 2+2+1, 5th tile orphaned. **Fix ([globals.css](app/globals.css)):** `.rpm-factor:nth-child(5):last-child { grid-column: 1 / -1; }` — spans the 5th tile full-width only when it's genuinely the 5th and last child (Order Wall is conditionally rendered, only appears with live order-book data); no-op in the 4-factor case. Verified live with real wall data present: 5th tile ("ORDER WALL") measured **325px** wide, matching the full container width exactly (vs. 160px for a normal half-width tile).

## C. Content / hierarchy — `/learn` glossary (#14)

### P2-3 — `/learn` reading hierarchy `[Medium]` — ⚠️ mostly MIS-MEASURED; small body bump applied
Re-measured accurately 2026-07-19 (`.learn-*` classes directly, not the first `h2`/`h3` on the page — the original probe grabbed the wrong `h2`): the hierarchy is actually **well-differentiated**, not flat. `.learn-category-title` (h2) is an **11px, weight-700, UPPERCASE, accent-blue eyebrow** label; `.learn-entry-term` (h3) is **16px bold white**; `.learn-entry-def` (body) was **13px** grey. So it's a clean eyebrow → title → body three-level structure — the "h2==h3==16px flat wall" claim was a flaky-pane artifact. **Applied fix:** the one legit tweak — bumped `.learn-entry-def` `--fs-label` (13px) → `--fs-body` (14px) for slightly more comfortable long-form reading on the SEO/education page ([globals.css](app/globals.css)). *Local render of this could not be confirmed (Turbopack dropped the globals.css edit even across a dev-server restart — the documented cache gotcha; `--fs-body` = 14px verified at the token level, so it renders 14px on the Render build).* No hierarchy change needed. `/learn` otherwise clean: 0 horizontal overflow, correct SEO title/H1, auth-aware CTAs.

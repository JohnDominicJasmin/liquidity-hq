...
| 8 | **Error and product tracking connected** (GlitchTip, PostHog, and the rest via MCP) | **high** | #1398 | **Error tracking PROVEN**: a real production exception reached GlitchTip in under a minute (`LIQUIDITYHQ-10`, release matching the deployed commit), and PII scrubbing was proven on the real pipeline. **Product analytics proven only to the network** — PostHog endpoints answer 200, dashboard arrival unconfirmed. Two minutes of owner time or a read-only key closes it |

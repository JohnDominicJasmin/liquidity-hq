...
**The sharpest version of that is not making a claim at all — it is repeating someone else's.** On 2026-09-07 a Dev commit body contained an aside: that a `paneId`-keyed removal would have taken the chart's own overlays, not just the sibling indicators. I restated it in a PR body I wrote for them. **In the commit it was a parenthetical. In the PR it was a labelled test step — "the widest blast radius in the change" — and QA read it as established and tested against it.** It was false: the library keeps indicators and overlays in separate stores and the removal has no path to the second one.

**The claim did not become more true on the way through this seat. It became more load-bearing.** Nothing was fabricated and the restatement was faithful; the promotion from aside to instruction is the entire error, and it is available to no other seat, because no other seat's output is a queue.

**Two habits fall out of it.** Mark which claims in a handoff you verified yourself and which you are relaying — a body that reads uniformly authoritative is one you have signed for. And **check the claim with an instrument that can answer it**: I read `IndicatorFilter` in the type definitions, saw `paneId` was a valid filter key, and stopped. **A type signature says what you may pass; it cannot say which store gets searched.** Dev opened the source. Same shape as `--green-2` and `.smod-panel` — declaration read, behaviour assumed.

**What saved it was QA testing the step anyway after being told the failure was structurally impossible.** Protect that instinct rather than optimising it away: an assurance deserves the same one-time control as a check does.

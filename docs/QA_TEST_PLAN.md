...
- [ ] As a trial user 2 days from expiry: does the reminder email actually arrive, and does its copy match what the trial actually loses? **Copy half DONE 2026-08-03; delivery half still open.** Delivery needs a real inbox and was not attempted - the disposable accounts use an undeliverable `@liquidityhq-internal.test` domain, and sending to a real address is an outward action worth asking about first. Copy was audited against every `LockedFeatureCard` call site and `lib/limits.ts` `GATED_TFS`, and **did not match**: it omitted the Confluence Score, on-chain scores, macro context and Dry Powder - four Pro features, one of which (Confluence Score) the FAQ leads on. Fixed in `lib/email.ts`; the list now matches what is actually gated. Gated timeframes (1m/5m/15m) already matched `GATED_TFS` exactly.
...
Layers 1-3: complete. Layer 4: 1 of 3 done, 2 blocked on access that does not exist yet (a second Telegram account, and the PostHog replay dashboard). Layer 5: 3 of 4 done, 1 half-done (trial email copy fixed, delivery unverified).

Everything still open is blocked on something external, not on effort:
...
- **Trial reminder email delivery (layer 5)** - needs a real inbox. Disposable accounts use an undeliverable domain on purpose, and mailing a real address is an outward-facing action to agree on first.

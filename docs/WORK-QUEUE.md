...
| # | Item | Size | Notes |
|---|---|---|---|
| Q1 | **Release #2 (#1252): verify on production** | hours | **Live since 2026-09-12 19:42Z** (`/api/version` reports `55a122c`, tagged `v2026.09.12b`). Both production DB changes went in first and were read back. The E2E run was red, but it was triaged, and none of the failures comes from the release (the table is on #1252). **Now re-check #1252's steps 1–4 on liquidity-hq.com.** Then **close release #2's issues once production is verified, not when they merge:** #1202, #1192, #1177, #1167, #1107 and #1059. #1077 stays open for #1233–#1238. **Release #1's signed-in production check waits on the owner's A or B** (`docs/OWNER-BLOCKERS.md` row 12). |

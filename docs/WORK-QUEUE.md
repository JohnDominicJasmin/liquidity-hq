...
| # | Item | Size | Notes |
|---|---|---|---|
| D1 | **#1222: finish the nav fix** | ~1 hour | **CLAIMED Dev 2026-09-12.** Signed out, it's audited clean at 768/800/850/944px in both themes, and signed-out `/settings` now carries the language control. **Open: the signed-in avatar looks clipped at 768px.** Measure the avatar's `getBoundingClientRect().right` against `window.innerWidth` at 768 and 800px signed in. `.tnav`'s own scroll width measures the nav against itself, so it can read 0 while the control is cut off. Visual, so the owner signs off the final set. |
| D2 | **#1251: log the path on `NoFallbackError`** | ~hours | Production logs the error with no path, so a 404 probe and a broken route look the same. Log the pathname. **Leave out query strings, which can carry tokens.** |
| D3 | **#1233–#1238: #1077's remaining server routes, Binance → Bybit** | ~half day each | agg-trades, funding-rate, rsi, snapshot, the proxy's ticker+depth, and the proxy's futures-derivatives. **Follow #1240's shape:** a whole series from one source, never spliced, with an honest source label. **#1249 is the trap to avoid: a fallback that gets an empty 200 fails loud and names the cause. It never caches that as a success.** Go in number order unless two routes share a helper. |
...
| # | Item | Size | Notes |
|---|---|---|---|
| Q1 | **Release #2 (#1252): the browser suite, then production** | hours | **#1252's E2E came back RED on 2026-09-12: 15 failed, 1 flaky, 609 passed. The table is on #1252. Triage it first:** run the 11 failing specs locally on `main` and on `f2f89f1d`, then post a per-spec verdict on #1252, pre-existing or regression. Regressions go to Dev. Stale specs, QA fixes. **CI is switched off until the re-run**, and PM/DevOps switches it back on for it. After the deploy, re-check #1252's steps 1–4 on liquidity-hq.com. Then **close release #2's issues once production is verified, not when they merge:** #1202, #1192, #1177, #1167, #1107 and #1059. #1077 stays open for #1233–#1238. **Release #1's signed-in production check waits on the owner's A or B** (`docs/OWNER-BLOCKERS.md` row 12). |
| Q2 | **Review dev's open PRs, in this order** | hours | #1250 (#1249), #1253 (#1246), #1245 (dialog focus), #1222 (after D1's avatar fix), then #1254 (docs). **#1220 and #1225 stay unmerged until the owner signs off their screenshots** (`docs/OWNER-BLOCKERS.md` row 11). |
| Q3 | **Tests for what merges** | per PR | QA owns every test (owner ruling, 2026-09-07). #1250 and #1253 each say what to assert, so **write those once they merge**, and the coverage gap closes the same day. |

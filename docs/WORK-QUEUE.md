...
| # | Item | Size | Notes |
|---|---|---|---|
| Q1 | **Release #2 (#1252): the browser suite, then production** | hours | #1252's E2E run is going now. **A red run blocks the merge, so post it there.** After the deploy, re-check #1252's steps 1–4 on liquidity-hq.com. Then **close release #2's issues once production is verified, not when they merge:** #1202, #1192, #1177, #1167, #1107 and #1059. #1077 stays open for #1233–#1238. **Release #1's signed-in production check waits on the owner's A or B** (`docs/OWNER-BLOCKERS.md` row 12). |
...
| Q4 | **#950: `layout.spec.ts` against a live run** | read-only | **CI is on for release #2**, so #1252's E2E run is the live result. Read `layout.spec.ts`'s outcome there **before CI goes back off**. |

...
| D1 | **#853 — rebuild `ArenaTerminal.tsx`** | — | **CLOSED 2026-09-07 — the owner stopped the rebuild outright, not paused.** Superseded the same-day "parked" correction below within hours: `components/ArenaTerminal.tsx` will not be built. QA verified the CURRENT `/arena` (desktop and mobile 390, `staging` 909d4d3) reads as unremarkable, not broken — #853's own open question, answered — which is part of why the owner stopped it: the current screen doesn't read as urgent to fix. **No longer blocks Q3 criteria 12-19 — see Q3, those criteria are moot, not blocked.** History kept rather than deleted: `arenaEvidence.ts`/`useViewport.ts` were never restored, `components/ArenaTerminal.tsx` never existed on any branch at any point this file described it as "in flight" or "parked." |
...
| D4 | **34 extensionless *value* imports in `lib/`** | ~half day | This — not a TypeScript loader — is what blocks unit-testing `marketStore`. Node 24 strips `.ts` types natively, so the documented reason was half wrong. **The raw count is 43 across 19 files, but nine are `import type` and are erased before Node resolves anything — the real sweep is 34.** Sizing from 43 over-sizes it; grepping for 43 afterwards looks like nine were missed. Re-measure before starting, this moves with `dev`. **No longer gated on #853** — closed 2026-09-07, won't land at all; the "do not start before it lands" note was written when #853 was still expected to touch overlapping files. Unblocked. |
...
| Q3 | **`TEST_GAPS.md` §2 — geometry is checked, appearance is not** | small, **narrowed 2026-09-07** | **First pass done, posted on #914.** Structure and geometry measured, all 45 live `at-*` classes matched a rule, colour-as-data confirmed against a real fired signal versus a real non-firing positive value. **Mobile 390: UNBLOCKED, not #990** — `resize_window` (the browser-extension tool) never changes the viewport, confirmed reproduced, but **Playwright's own context viewport is real and independent of it** — `{width:390,height:844}` gives a genuine `innerWidth===390` with real media-query evaluation, checked directly. Used it to answer #853's mobile question same-day; the same path closes this criterion. **Entitled/pro view: CLEARED 2026-09-07**, full PRO content throughout against the confirmed-PRO fixture, no gap. **Non-BTC clusters: resolved** — #921 was a real wiring defect (`LiqFeed` mounted only in the current-design branch), reverted with the Arena bundle, structurally fixed since; still empty empirically and #925 carries it. **Criteria 12-19: MOOT, not blocked — #853 closed 2026-09-07, not paused.** They asserted against markup only `ArenaTerminal.tsx` would emit, and that component will not be built. Nothing to scope, stub, or wait on; drop these criteria from the section rather than continuing to track them as pending. |

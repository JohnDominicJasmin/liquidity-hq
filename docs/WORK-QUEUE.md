...
| D1 | **#853 — rebuild `ArenaTerminal.tsx`** | ~1 week | **REOPENED on the owner's instruction, 2026-09-07 — same day it was reported closed.** The closure itself was reported in error (PM/DevOps: "the churn was mine, not the owner's") and voided within the same day; recorded rather than silently reverted, since this file said "CLOSED" for part of that day and a reader deserves to know why the wording moved twice. Back to its prior standing: `components/ArenaTerminal.tsx` does not exist on any branch. Densest screen, 15 modules, restyle not restructure. `arenaEvidence.ts` and `useViewport.ts` need restoring from `dd39c9bb^` — removed by a *different* revert (`c4921954`). `useViewport` exports `LANDING_MOBILE_QUERY`, so the `docs/HANDOVER.md:383` landing gate applies and is verified by **rendering**, not by reading the diff. **Blocks QA's Q3 criteria 12-19 again**, which assert against markup only this component emits. QA's same-day finding that the CURRENT `/arena` reads as unremarkable, not broken (desktop and mobile, `staging` 909d4d3, posted on #853) stands regardless of this reversal — it answered #853's own open question, it did not argue against rebuilding. |
...
| Q3 | **`TEST_GAPS.md` §2 — geometry is checked, appearance is not** | ~1 day, **partly blocked** | **First pass done, posted on #914.** Structure and geometry measured, all 45 live `at-*` classes matched a rule, colour-as-data confirmed against a real fired signal versus a real non-firing positive value. **Mobile 390: UNBLOCKED, not #990** — `resize_window` (the browser-extension tool) never changes the viewport, confirmed reproduced, but **Playwright's own context viewport is real and independent of it** — `{width:390,height:844}` gives a genuine `innerWidth===390` with real media-query evaluation, checked directly. Used it to answer #853's mobile question same-day; the same path closes this criterion whenever it's picked up — this correction stands regardless of #853's own status. **Entitled/pro view: CLEARED 2026-09-07**, full PRO content throughout against the confirmed-PRO fixture, no gap. **Non-BTC clusters: resolved** — #921 was a real wiring defect (`LiqFeed` mounted only in the current-design branch), reverted with the Arena bundle, structurally fixed since; still empty empirically and #925 carries it. **Criteria 12-19 are a #853 DEPENDENCY again** — #853 reopened 2026-09-07 after a same-day report that it had closed (see D1); back to needing `arenaEvidence.ts`'s fixture-injection stub, which cannot be scoped until the component exists. `lib/arenaEvidence.ts` and `components/ArenaTerminal.tsx` **do not exist anywhere** — not `dev`, not `staging`, not the live route. **Whether evidence arrives as an injectable prop or a hookless read cannot be decided until the component exists**, so the stub cannot be scoped, let alone built. |

...
| # | Item | Size | Notes |
|---|---|---|---|
| D1 | **#1245: UsageModal drops focus on close** | ~hours | QA found it on #1245. "View Usage" closes the dropdown in the same click that opens the modal, so the button that opened it unmounts, and on Escape focus falls to `<body>`. **It fails on every real open.** Return focus to something that still exists, such as the avatar button that owns the menu, and say in the PR what QA should assert. The other three dialogs passed. |

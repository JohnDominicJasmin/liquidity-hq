...
| # | Item | Size | Notes |
|---|---|---|---|
| D00 | **#1049 — a PRO SUBSCRIBER IS TREATED AS FREE on the rebuilt Arena. Blocks the rebuild reaching `main`.** | ~hours | `entitled` resolves **false** on `/arena` for a genuinely entitled session. Consequence is not just the `NEED PRO` hint: **gated timeframe chips, and `ConfluenceScore` replaced by `LockedFeatureCard`** — a paying customer opens the page they pay for and is told to upgrade. **Confirmed real, not a fixture artefact, and QA ruled out the obvious explanation before filing.** Dev's hypothesis was that the test session only looked signed in (precedent: #831). QA redid it with `useEntitledSession(page, {as:'pro'})` — the trusted mechanism — same result; then proved the fixture itself sound by running `entitled-macro-cards.spec.ts` through it against the dashboard, which passes cleanly. **Arena-specific, not fixture-wide.** Reproduces on deployed staging as well as locally. **`lib/arenaTimeframes.ts` is correct** and both sessions agree on that — the break is upstream, in whatever `app/arena/page.tsx`'s `useAuth()` resolves `entitled` to on this page. Root cause not yet found, and QA said so rather than guessing. **Nobody is affected today, checked before reporting:** production serves `8f7734f`, which predates ArenaTerminal entirely. **It becomes customer-facing the moment the rebuild reaches production.** **Sequencing: the CSS branch merges first** (it fixes a visible regression on `qa`/`staging`), **then this, as its own fix.** Bundling two defects with two causes into one PR is how #1039 went wrong. **Nothing reaches `main` until this closes.** **And the check that should have caught it:** #853's sign-off verified the signed-out **free** path and passed. **The Pro path on the rebuilt page was never exercised** — criterion 5 says all 15 modules render for an entitled user, and that half went unrun. Second hole in the criteria after the pixel gap, and the more expensive one. |

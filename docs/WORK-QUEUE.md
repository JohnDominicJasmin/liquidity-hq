...
| # | Item | Size | Notes |
|---|---|---|---|
| Q1 | **#243 harness — everything up to the LemonSqueezy handoff** | ~1 day | **DONE — #917 merged 2026-09-06.** Config agreement, C's baseline and the checkout hand-off all run; the entitlement pair (row + rendered gate) and #134's cancel-keep-until-expiry are written and skip cleanly until a real purchase exists for C. **Open, raised in review:** the gate it asserts against on `/dashboard` is `GlobalMacroContext`'s component-level gate, not a page-level one — discriminating today, but only while that component stays on that page. A control asserting the card *is* present for a free account would anchor it. |
| Q2 | **`TEST_GAPS.md` §10 — monitoring delivery is unconfirmed** | ~half day | **Reframed as a read, not a write (#918, merged) — then BLOCKED.** Neither QA nor PM/DevOps has Sentry/PostHog dashboard access. Needs the owner to check one number on each (any event ever received, prod project/environment) before this moves. The distinction stands: prod reports `sentry: true`, `posthog: true`, and configured ≠ working — same shape as `gh workflow list` reporting `active` for a workflow that had not run in 22 days. |
| Q3 | **`TEST_GAPS.md` §2 — geometry is checked, appearance is not** | ~1 day | **First pass done, posted on #914.** Rail 352, nav 44, ticker 34; heatmap confirmed struck; **45 live `at-*` classes, all 45 matching a rule**; colour-as-data confirmed on a real fired signal (OI 1h green in value and marker) against a real non-firing positive number (Funding 8h, correctly neutral). **The empty non-BTC clusters ladder was a wiring defect, not inconclusive** — `LiqFeed` was mounted only below the current-design return, fixed in #921. Still open: mobile 390, the entitled/pro view, and criteria 12-19, which need an `arenaEvidence` fixture stub that does not exist. **And `arena-structure.spec.ts` asserts far less than its test names claim** — three tests, five `expect()`s: rail width, nav, ticker, mobile rail. Criteria 1, 3 and 4 are unasserted despite the first test being named for them. |

...
| # | Item | Size | Notes |
|---|---|---|---|
| D0 | **#967 — the three `pane:'candle'` builtins render into a phantom pane** | ~hours | **Ahead of everything. QA reproduced live 2026-09-07.** `KLineProChart.tsx:2213` passes `{ id: 'candle_pane' }` where the argument is `CreateIndicatorOptions` — which has `isStack`, `pane`, `yAxis` and **no `id`**. Fresh pane allocated; removal then filters on a pane the indicator was never in, so the pane persists all session. Correct shape is one ternary arm away. **Fixing the shape alone is not enough** — `IndicatorFilter` matching on `paneId` hits *every* indicator on that pane, so three builtins correctly sharing the candle pane means deselecting one removes all three. Key removal on the id `createIndicator` returns. Also handle its `null` return: an unknown name does not throw, so the `catch` cannot cover it. **Remove the `as any` on line 2210** — excess property checking would have rejected the bad object. |
| D0b | **#970 — glow tokens composed into a `box-shadow` list** | ~half day | Behind D0. `box-shadow: …, var(--glow-x)` where the token is `none` is invalid, so the **whole declaration** is dropped. Five tokens, three used in a list. **Census first, and it must be a count and not a bound.** No rendering changes in this pass — terminal is settled (no shadows), current design is one owner decision over the whole list, not five. |
...
| # | Item | Size | Notes |
|---|---|---|---|
| Q0 | **Re-test #967 once D0 lands** | ~hours | **Two things the obvious test will miss.** First, deselect *one of several* sharing the candle pane — fixing the option shape trades a visible zombie pane for silent collateral removal, which is strictly harder to see, so verify the behaviour rather than the diff. Second, deselect in **several orders**, not just selection order: the map is written on add and read on remove, and order is where that goes wrong. |
| Q1 | ~~**#969 — fourth a11y batch**~~ **Merged 2026-09-07**, all four sites verified live including the Arena history row `div`→`span` restructure by direct visual check. | done | Kept as a row rather than deleted — removing a closed item is how a queue loses its own history. |

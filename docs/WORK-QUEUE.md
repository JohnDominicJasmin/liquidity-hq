...
| **#1059 / #1077 — Binance to Bybit** | Client-side failover and server-side egress. Distinct scopes, deliberately not merged. **#1059 CLAIMED Dev 2026-09-12**, PR #1228 open and unmerged - all review points addressed, waiting on a local-server pass. **#1077 CLAIMED Dev 2026-09-12** - scoped on the issue: `klines` route first (highest value, both exchanges' handling already coexist in that file), `agg-trades`/`funding-rate`/`rsi`/`snapshot`/`proxy`'s 6 Binance-only endpoint types are separate follow-ups, each its own response-shape design, not bundled into one PR. |

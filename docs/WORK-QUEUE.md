...
| PR-FU1 | **#1309 follow-up: FUNDING_SIG_LONGS_OVERCROWDED_DESC neutral ending + funding-payer extraction** | ~small | **Built (PR #1331), gates green** (lint/tsc/892 tests/build). Mirrors the SHORTS twin's neutral funding-tip ending; `isFundingPaying`/`isHighFundingRateWarning` extracted to `lib/fundingCost.ts`. Migration's prod section ready, not applied. Awaiting QA. |

...
Owner/staff-only operations console at `/ops` (login at `/ops/login`; old `/admin` path is a honeypot that logs the probe and 404s). Not part of any pricing tier — internal tooling. Full status + backlog in `pendings/pendings/OPS_ROADMAP.md`.
...
Add an admin (creates a pre-confirmed Supabase user or grants an existing account access), change role, disable/remove. Best-effort "you've been added" email via Brevo (`lib/email.ts`) — unreliable without a verified domain, see `pendings/pendings/OPS_ROADMAP.md`.

...
| 8 | **Restart the dev Supabase project**: the dashboard's Project Settings → General → Restart project | Dashboard-only (no tool in the team's reach can restart a project) and disruptive to `dev`, `qa` and `staging` at once. | **ASKED 2026-09-12 ~09:45Z, not yet actioned.** Degraded since **09:16:24Z**, when PM/DevOps's additive column for #1202 set off a PostgREST schema-reload outage (#1025). It spread to auth by 10:04Z. Postgres itself is idle, using 20 of 60 connections with no waiting locks, while every client times out, which points at the instance itself. **What it costs:** release #1210's staging pass (owner-approved on QA's sign-off) and its E2E run are void and cannot re-run. Dev cannot verify #1217 or screenshot #1220. **The whole team is blocked on the environment, not on work.** **Delete this row once the restart is done and 5 clean minutes are confirmed.** |

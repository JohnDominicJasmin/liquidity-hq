...
| 11 | **Visual sign-offs waiting (condition 5)** | Visual. | **Screenshots sent 2026-09-12** for **#1244** (the chart's data-source label), **#1220** (page titles on mobile) and **#1225** (the signup loading state). **#1222** follows once its final-CSS shots land: the nav at 768–899px, where theme and language move to `/settings` and item padding tightens. **None of these issues can close without the owner's sign-off.** The code can still ship, since condition 5 gates closing, not merging. |

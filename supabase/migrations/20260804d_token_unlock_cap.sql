-- token-unlock/route.ts relied entirely on the shared response cache
-- (lib/apiCache.ts) for cost control, keyed on the user-supplied symbol after
-- only a loose length check (2-10 chars, any charset). Any signed-in user
-- could defeat the cache and force a fresh Grok call every time by varying
-- the symbol text (whitespace, punctuation, casing beyond what's stripped) -
-- no per-user daily cap existed on this route at all, unlike the other AI
-- tools. Adds a token_unlock_count column and wires it into the same
-- increment_ai_usage() atomic function used everywhere else - see
-- 20260804c_atomic_ai_usage_increment.sql for why it's atomic (closes the
-- TOCTOU race) and why the column is matched via a fixed CASE list.
--
-- Run the PROD block below in project qdpwhnvmhqgzijuwopso, the DEV block
-- (commented) in project wdtjhrilakoitfcezxpx.

-- ── PROD ─────────────────────────────────────────────────────────────────
alter table lhq_grok_usage
  add column if not exists token_unlock_count int not null default 0;
...
--       returning hypothesis_analyze_count into v_new;
--   elsif p_column = 'token_unlock_count' then
--     update lhq_dev_grok_usage set token_unlock_count = token_unlock_count + 1, updated_at = now()
--       where user_id = p_user_id and date = p_date and token_unlock_count < p_limit
--       returning token_unlock_count into v_new;
--   else
--     raise exception 'increment_ai_usage: invalid column %', p_column;
